...
const { runMemoryCommand, runHistoryCommand, runEvolveCommand } = require('../core/memory');
...
function buildCommandHandlers(): Record<string, CommandHandler> {
  const handlers: Record<string, CommandHandler> = {
    ...
    history: args => runHistoryCommand(args[0], args.slice(1)),
...
function main(argv: string[]): void {
  ...
  const readOnlyCommands = new Set(['memory', 'history', 'guidance', 'help', '--help', '-h']);

  if (command !== 'finish' && !readOnlyCommands.has(command)) {
...
export function runShortcut(shortcut: string, args: string[]): void {
  ...
  if (shortcut === 'history') return run(['history', 'view', ...args]);
