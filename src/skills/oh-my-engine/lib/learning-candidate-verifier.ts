...
function verifyLearningCandidate(projectRoot: string, slug: string): MemoryRecord {
  ...
  if (errors.length > 0) {
    return updateLearningCandidateRecord(projectRoot, slug, (current: MemoryRecord) => ({
      ...current,
      verification: {
