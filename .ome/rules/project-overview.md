...
version: 1.0.0
description: Repository profile for oh-my-engine
...
- Treat `.ome/context/project-scan.json` as the machine-readable baseline, then inspect the current source before making architecture claims.
- Prefer rules that name actual directories, entrypoints, scripts, dependencies, and framework signals from this repository.
- Keep generated platform files derived from `.ome/rules/` through `ome rules sync`.
