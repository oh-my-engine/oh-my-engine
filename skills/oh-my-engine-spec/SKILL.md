...
- `apply`：输出实现上下文、已采纳 skill 的 execution directives，并支持勾选任务/验收项回写进度
- `status`：汇总当前 phase、任务计数、engine-memory context 和 execution directive 数量
...
`apply` 不会自动改业务代码；它会把已采纳 skill 转成必须遵循的 execution directives，直接作用于实现提示。
...
- 验证策略
- 输入分析里的未决问题
- 刷新 `context/engine-memory.md`，把已采纳的 learnings / generated skills 注入当前 change 上下文
- 直接输出 adopted skill 的 execution directives，约束后续实现方案
