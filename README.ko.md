...
- Codex 스킬 이름: `oh-my-engine-init`, `oh-my-engine-ui`, `oh-my-engine-bug`, `oh-my-engine-comp`, `oh-my-engine-api`, `oh-my-engine-spec`, `oh-my-engine-memory`, `oh-my-engine-evolve`
...
`.oh-my-engine/config.json`의 `workflows.spec.options.verifyCommands` 아래에 실제 프로젝트 검사를 추가할 수 있습니다. `verify`는 이를 순차적으로 실행하고, 첫 번째 비정상 종료 코드에서 실패합니다. 또한 `verify`는 해결되지 않은 `TBD:` 마커를 차단하고, 각 spec delta가 정확히 하나의 변경 유형을 선택하며 최소 하나의 구체적인 requirement와 WHEN/THEN 시나리오를 포함하도록 요구합니다.
...
├── oh-my-engine-spec/     # OpenSpec 호환 spec 워크플로우
├── oh-my-engine-memory/   # 메모리 뷰어
└── oh-my-engine-evolve/   # 진화 분석기

project/
├── .oh-my-engine/         # 프로젝트별 구성 및 메모리
