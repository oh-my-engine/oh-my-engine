...
```
请帮我安装 Oh My Engine，这是一个为 Claude Code 设计的自我进化工作流框架。
...
```
Claude Code 用の自己進化型ワークフローフレームワーク「Oh My Engine」のインストールを手伝ってください。
...
```
Claude Code를 위한 자체 진화 워크플로우 프레임워크인 Oh My Engine 설치를 도와주세요.
...
```
Por favor ayúdame a instalar Oh My Engine, un framework de flujo de trabajo auto-evolutivo para Claude Code.
...
```
Aidez-moi à installer Oh My Engine, un framework de workflow auto-évolutif pour Claude Code.
