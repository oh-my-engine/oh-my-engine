...
从 0.5.0 版本开始，`ome evolve` 对生成或采纳的技能候选执行严格的质量检查，防止生成泛泛建议型技能。

### 必需章节

所有技能候选必须包含：

- **Purpose** - 解决什么问题，产生什么价值
- **When to Use** - 明确触发条件和适用场景
- **Inputs** - 需要的用户输入、项目文件、上下文
- **Process** - 可执行的步骤，不是泛泛建议
- **Red Flags** - 必须停下来重新诊断的信号
- **Common Rationalizations** - 阻止 Agent 偷懒的反借口规则
- **Verification** - 完成前必须收集的证据
- **Output Contract** - 最终回复必须包含的内容

### 评分维度

系统从六个维度评分（每项 0-5 分）：

- **specificity** - 具体性：是否有明确触发条件和可执行步骤
- **actionability** - 可操作性：是否能直接执行，而非泛泛建议
- **verification** - 可验证性：是否有明确验证要求和证据收集
- **scope_control** - 范围控制：是否防止任务扩散和无关修改
- **reuse_of_existing_patterns** - 复用现有模式：是否优先使用项目已有模式
- **risk_awareness** - 风险意识：是否识别常见失败模式和红旗

### 采纳规则

- 总分低于 24/30：不能 adopt
- `verification` 低于 4：不能 adopt
- `scope_control` 低于 4：不能 adopt
...
详见 [docs/skill-quality-gate.md](skill-quality-gate.md) 和 [docs/skill-anatomy.md](skill-anatomy.md)。
...
6. ✅ **效果跟踪**：持续优化
7. ✅ **自动清理**：保持系统健康
8. ✅ **质量门槛**：确保生成技能结构化、可验证、可执行

系统设计原则：
- **自主但可控**：自动化常规操作，人工审核关键决策
- **学习但不盲目**：基于证据，持续验证
- **进化但不混乱**：自动清理，保持整洁
- **严格但实用**：质量门槛防止泛泛建议，保证可执行性
